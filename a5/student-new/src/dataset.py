...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2    
        # [prefix] [suffix] [masked_content] [pads]
        len_masked_content = int(trunc_len * random.gauss(mu=1/4, sigma=0.1))
        len_masked_content = np.clip(len_masked_content, 1, trunc_len-2)
        masked_content_idx = random.randint(1, trunc_len - len_masked_content)
        masked_content = document[masked_content_idx:masked_content_idx + len_masked_content]
        prefix = document[0:masked_content_idx]
        suffix = document[masked_content_idx + len_masked_content:]
        # 3
