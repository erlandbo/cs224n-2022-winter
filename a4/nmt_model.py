...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = torch.nn.LSTMCell(input_size=hidden_size+embed_size, hidden_size=hidden_size, bias=True)
        ...
        self.combined_output_projection = torch.nn.Linear(in_features=3*hidden_size, out_features=hidden_size, bias=False)
        self.target_vocab_projection = torch.nn.Linear(in_features=hidden_size, out_features=len(vocab.tgt), bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 3.
        init_decoder_state = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), dim=1))
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), dim=1))
        dec_init_state = (init_decoder_state, init_decoder_cell)
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_state = self.decoder(Ybar_t, dec_state)  # 1.
        dec_hidden, dec_cell = dec_state  # 2.
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(-1)).squeeze(-1)  # 3.
        ### END YOUR CODE
