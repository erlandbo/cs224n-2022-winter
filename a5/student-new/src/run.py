...
if args.variant == 'vanilla':
    ...
    mconf.synthesizer = True  # TODO [part g]: Make some other model here
...
if args.function == 'pretrain':
    ...
    corpus = open(args.pretrain_corpus_path).read()
    corpus_dataset = dataset.CharCorruptionDataset(corpus, block_size)
    trainer = trainer.Trainer(model, corpus_dataset, None, tconf)
    trainer.train()
    ...
    if args.reading_params_path:
        ...
        tconf = trainer.TrainerConfig(
            max_epochs=10, batch_size=256, learning_rate=6e-4,lr_decay=True, 
            warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,num_workers=4
        )
    else:
        tconf = trainer.TrainerConfig(
