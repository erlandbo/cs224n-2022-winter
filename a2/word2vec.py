...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    num_words, word_len = outsideVectors.shape
    y_hat = softmax(outsideVectors @ centerWordVec)
    loss = - np.log(y_hat[outsideWordIdx])
    y = np.zeros_like(y_hat)
    y[outsideWordIdx] = 1
    gradCenterVec = outsideVectors.T @ (y_hat - y)
    gradOutsideVecs = (y_hat - y)[:, np.newaxis] @ centerWordVec[:, np.newaxis].T
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    U = -1 * outsideVectors[indices]  # (K+1) x d
    U[0] *= -1
    u_o, v_c = outsideVectors[outsideWordIdx], centerWordVec  # d,
    loss = -np.sum(np.log(sigmoid(U @ v_c)))  # 1,
    dU = (1-sigmoid(U @ v_c))[:, np.newaxis] @ v_c[:, np.newaxis].T  # (K+1)x1 @ 1xd -> (K+1)xd
    dU[0] *= -1
    gradOutsideVecs = np.zeros_like(outsideVectors)
    np.add.at(gradOutsideVecs, indices, dU)
    gradCenterVec = np.sum(U * -1*(1 - sigmoid(U @ v_c))[:, np.newaxis], axis=0)  # (K+1)xd * (K+1)x1 -> (K+1)xd * (K+1)xd
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w_j in outsideWords:
        # if w_j is currentCenterWord: continue
        loss_wj, gradCenterVec, gradOutsideVector = word2vecLossAndGradient(v_c, word2Ind[w_j], outsideVectors, dataset)
        loss += loss_wj
