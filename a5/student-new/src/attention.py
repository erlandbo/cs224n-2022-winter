...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        att = F.relu(self.w1(x)).view(B, T, self.n_head, C//self.n_head).permute(0,2,1,3)  # (B,T,C) -> (B,T,nh,hs)->(B,nh,T,hs)
        att = att @ self.w2[:, :T] + self.b2[:T]  # (B,nh,T,hs) @ (hs,T) + (T,) -> (B,nh,T,T) + (T) -> (B,nh,T,hs)
        # alt.  (B,nh,T,hs) @ (1,1,hs,T) + (1,1,1,T) -> (B,nh,T,T) + (1,1,1,T) -> (B,nh,T,hs)
        att = att.masked_fill(self.mask[:, :, :T, :T] == 0, float('-inf'))  # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        y = att @ v  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
