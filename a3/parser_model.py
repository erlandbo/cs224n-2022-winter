...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, torch.flatten(w)).view(batch_size, -1)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        h = F.relu(x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias) # (batch_size x n_features * embed_size) @ (embed_size*n_features x hidden_size) + hidden_size,
        h = self.dropout_layer(h)
        logits = h @ self.hidden_to_logits_weight + self.hidden_to_logits_bias # batch_size x hidden_size @ hidden_size x logits_size + logits_size,
        ### END YOUR CODE
        return logits
